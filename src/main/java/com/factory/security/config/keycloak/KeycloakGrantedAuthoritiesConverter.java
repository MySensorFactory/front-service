...
public class KeycloakGrantedAuthoritiesConverter implements Converter<Jwt, Collection<GrantedAuthority>> {
    ...
    private Collection<GrantedAuthority> defaultGrantedAuthorities(final Jwt jwt) {
        return Optional.ofNullable(defaultAuthoritiesConverter.convert(jwt))
                .orElse(emptySet());
    }

    @SuppressWarnings("unchecked")
    protected List<String> realmRoles(final Jwt jwt) {
    ...
    @SuppressWarnings("unchecked")
    protected List<String> clientRoles(final Jwt jwt, final String clientId) {
